...
const VERIFIED_WALLET_MAP = {
  mepp: '0x6d9fc316c3b8377060a44b852ba664adbfd59790'
};
...
const resolveUsernameToWallet = async (username) => {
  ...
  if (VERIFIED_WALLET_MAP[key]) {
    return { address: VERIFIED_WALLET_MAP[key], user: { username: cleanUsername }, resolverUrl: 'verified-seed' };
  }
  ...
  for (const url of urls) {
    ...
    if (address) return { address, user, resolverUrl: url };
  }
  ...
  if (input.isUsername && !input.isAddress) {
    ...
    if (resolved?.address) {
      ...
      return {
        ...
        source: resolved.resolverUrl === 'verified-seed' ? 'verified-seed' : 'username-resolved',
